这是一个基于 `CMake` 的 `c` 语言 `Hello,World!` 例子，通过该例子来初步认识 `CMake` 的用法。

进入项目根目录，运行 `cmake` 的命令：
...
`cmake -G` 用来指定基于什么平台编译，可以执行 cmake --help 来看都有哪些平台。

```bash
PS C:\Users\freer> cmake --help
Usage
...
执行完 `cmake -G Ninja ..` 后，就可以用 `cmake --build .` 来编译生成可执行文件或库文件了。
