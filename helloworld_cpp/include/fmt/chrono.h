...
public:
    FMT_CONSTEXPR auto parse(basic_format_parse_context<Char>& ctx)
        ...
        if (*it == '.') {
            ...
            it = detail::parse_precision(it, end, specs_.precision,
                                         precision_ref_, ctx);
        }
        if (it != end && *it == 'L') {
            localized_ = true;
    ...
    auto format(std::chrono::duration<Rep, Period> d, FormatContext& ctx) const
        ...
        if (begin == end || *begin == '}') {
            ...
            using chrono_formatter =
                detail::chrono_formatter<FormatContext, decltype(out), Rep,
                                         Period>;
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>,
    ...
    auto format(
        ...
        if (detail::const_check(
            ...
            if (subsecs.count() < 0) {
                auto second = detail::fmt_duration_cast<Duration>(
            ...
            return formatter<std::tm, Char>::do_format(gmtime(val), ctx,
                                                       &subsecs);
        }
...
struct formatter<std::chrono::local_time<Duration>, Char>
    ...
    auto format(std::chrono::local_time<Duration> val, FormatContext& ctx) const
        ...
        if (period::num != 1 || period::den != 1 ||
            ...
            return formatter<std::tm, Char>::do_format(localtime(val), ctx,
                                                       &subsecs);
        }
